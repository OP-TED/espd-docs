...
All the semantic assets needed to implement ESPD electronic documents are organised in a link:{distUrl}/dist[distribution package]. The content of this folder is organised as follows:
...
. *xslt*: transformation stylesheets used to generate basic sample XML instances from the criteria defined in spreadsheets link:{attachmentsdir}/dist/cl/ods/ESPD-Data_Structures-REGULATED-V02.00.00.ods[*REGULATED* ESPD data structures] and link:{attachmentsdir}/dist/cl/ods/ESPD-Data_Structures-SELFCONTAINED-V02.00.00.ods[*SELF-CONTAINED* ESPD data structures] (see folder link:{distUrl}/dist/cl[dist/cl]).
