...
. In an Office Open XML (link:{attachmentsdir}/cl/xlsx/ESPD-CodeLists-V2.1.0.xlsx[XLSX]) file,
. In an Open Office Document (link:{disturl}/dist/cl/ods[ODS]) file,
. Genericode 1.0 (link:{disturl}/dist/cl/gc[GC]) files (one `*.gc` file per code list).
