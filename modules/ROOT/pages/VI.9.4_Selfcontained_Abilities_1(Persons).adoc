...
link:{url-blob}/dist/cl/xlsx/ESPD-CriteriaTaxonomy-SELFCONTAINED-V2.1.0.xlsx[ESPD-CriteriaTaxonomy] spread-sheet).

In this data structure the first REQUIREMENT set by the contracting authority is whether the criterion is to be WEIGHTED or not (the opposito to WEIGHTED is PASS/FAIL, see the Code List link:{url-blob}/dist/cl/ods/ESPD-CodeLists-V2.1.0[EvaluationMethodType]).
...
. The data structure (see the SELF-CONTAINED spread-sheet tab link:{url-blob}/dist/cl/ods/ESPDRequest-CriteriaTaxonomy-SELFCONTAINED-V2.1.0.ods[SC-Abilities_1]).
