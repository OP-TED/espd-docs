...
. The URL where to access the evidence. Will be placed in an element of the object cac:Evidence. See chapter "xref:xml_technical_handbook.adoc#6-8-evidences[6.8 Evidences]".
. By specifying that the value of the expected data type is an evidence identifier (code EVIDENCE_IDENTIFIER of the Code List ResponseDataType`) the Response will have to contain an evidence supplied in an element of the object cac:Evidence, at the end of the XML Instance. See chapter "xref:xml_technical_handbook.adoc#6-8-evidences[6.8 Evidences]".
...
. Compulsory name and description for the criterion as defined in e-Certis. See also link:{attachmentsdir}/dist/cl/ods/[ESPD-criterion-V3.0.0 spreadsheet]. Beware that UBL-2.3 allows '0..n' description lines!
. This property is a caption, a mere 'label'. See Code List link:{url-tree}/dist/cl/ods/['CriterionElementType'] for the complete list of possible options. Beware that when a property is of type CAPTION the cbc:ValueDataType must be set to NONE.
