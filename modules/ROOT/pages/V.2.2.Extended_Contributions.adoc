...
<4> Notice that type of property used is REQUIREMENT. This entails that: (i) the ESPDRequest is of type EXTENDED (BASIC ESPDs does not work with REQUIREMENT(s)); (ii) a `cbc:ValueDataType` will be present indicating the type of value the contracting authority uses for this REQUIREMENT (e.g. AMOUNT, INDICATOR, DESCRIPTION, etc.; see the link:{url-blob}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[Code List `ResponseDataType`], the values are the same for contracting authorities REQUIREMENT(s) than for economic operator response values).
