...
The ESPD-EDM version 4.0.0 main change is the replacement of dynamic UUIDs with XML Path like IDs. Some other minor errors were fixed in the data structure and the codelists.   
...
** **criterion** folder created. It contains the data structure of the ESPD taxonomy criterion Excel files for the Request and the Response. It also contains two examples in Excel format illustratign multiple occurences: one for the ESPD Request and one for ESPD Response;
...
Introduction of ESPD-EDM example files in Excel format together with the ESPD-EDM criterion file that can be found in **criterion** folder. +
