...
include::VIII.6.1_Basic_Economic_Operator_Party.adoc[]
include::VIII.6.1.1_Basic_Economic_Operator_XML.adoc[]
include::VIII.6.2_Extended_Economic_Operator_Party.adoc[]
include::VIII.6.2.1_Extended_Economic_Operator_XML.adoc[]
...
include::VIII.7.0_Answers.adoc[]
include::VIII.7.1_Answers_Elements.adoc[]
include::VIII.7.2_Simple_Request-Response_XML.adoc[]
include::VIII.8.0_Evidences.adoc[]
include::VIII.8.1_Evidences_Elements.adoc[]
include::VIII.8.2_Evidences_XML.adoc[]
////
...
For the ESPD Response, the ESPD-EDM V2.1.1 uses the UBL-2.2 document named `QualificationApplicationResponse.xsd` XSD Schema. This schema can be found under the folder link:{url-tree}/dist/xsdrt/maindoc[dist/xsdrt/maindoc] (or the equivalent documented `xsd` folder).
...
|*File*: |link:{url-blob}/dist/xsdrt/maindoc/UBL-QualificationApplicationResponse-2.2.xsd[dist/xsdrt/maindoc/UBL-QualificationApplicationResponse-2.2.xsd]
...
*Rule*: Compulsory use of the codelist link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[QualificationApplicationType] (values 'BASIC`, `EXTENDED`). Compulsory use of attributes listID, listAgencyName and listVersionID.
...
The XML snippet below shows how the beginning of an ESPD Response XML instance looks like. For a complete instance of an ESPD Response XML document see the the example files in the `dist/xml` folder: link:. or link:{url-blob-211}/dist/xml/EXTENDED-ESPDResponse-2.1.1.xml[EXTENDED-ESPDResponse-2.1.1.xml].
...
The UBL-2.2 element for the role of the economic operator is quite straightforward and typical in UBL: it provides a pair code + description (see Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EORoleType] for the codes and descriptions; see also the XML example below).
...
[cols="<1,<4"]
|===
|*REQUIREMENT*|For the identification of the economic operators the UBL-2.2 element `cac:PartyIdentification/cbc:ID` must be used. The following two attributes of the cbc:ID element (CCT IdentifierType element) must be used: (1) `@schemeID`, a code from the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EOIDType] must be used; (2) `@schemeAgencyID`, the default value "EU-COM-GROW" must be used; (3) Optionally, the attribute `@schemeName` can be used to place the 'Name' of the code (see the column 'Name' in the above mentioned Code List). For the ESPD-EDM the preferred type code for the `@schemID` is VAT.
|===	
...
** *cac:QualifyingParty/cac:Party/cbc:IndustryClassificationCode* to indicate whether the EO is a micro, small, medium or large company (or simply an SME). This code is defined in the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EOIndustryClassificationCode]. See also sections "Expected Elements" and "XML example" for more details.
...
*Rule*: Use it to place here the general Turnover of the EO (for statistical purposes). Compulsory assignment of a value to the attribute currency. The default value should be set to 'EUR'. Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[CurrencyCode].
...
*Rule*: Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EORoleType].
...
*Rule*: Software applications should retrieve and reuse the description from the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EORoleType].
...
*Rule*: The country of the contracting body must always be specified. Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[CountryCodeIdentifier] (ISO 3166-1 2A:2006).
...
*Rule*:  The country of the service provider must always be specified. Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[CountryCodeIdentifier] (ISO 3166-1 2A:2006).
...
<13> Code to identify the type of the company (micro, small, medium, SME, Large). Notice the use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EOIndustryClassificationCode] (for statistical purposes).
...
link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[EOIDType].
...
*Rule*: Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[CountryCodeIdentifier].
...
*Rule*: Try to use the name provided in the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[CountryCodeIdentifier] and in the language of the user.
...
The `cac:ApplicablePeriod` is used to hold the start date and the end-date provided by the economic operator to a QUESTION for which the expected data is of type 'PERIOD'. (See expected types in the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[ResponseDataType]).
...
See the codes used for `cac:ValueDataTypeCode` in the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[ResponseDataType]. For the different possible values in the response see the XSD diagrams above and the list of the expected elements in `cac:ResponseValue` in the table below.
...
*Rule*: Compulsory use of the Code List link:{url-blob-211}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[ConfidentialityLevel]. Software application should set this code to CONFIDENTIAL automatically when the confidentiality level code of at least one criterion to which this evidence is associated is set to CONFIDENTIAL.
