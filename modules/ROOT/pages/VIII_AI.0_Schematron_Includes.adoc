...
The ESPD-EDM uses Schematron to validate business rules identified in link:{url-tree}/dist/doc[ESPD_TestCases_v10.docx and ESPD_TestCases_mapping_v10.docx documents]. The input for these business rules is the e-Sens link:http://wiki.ds.unipi.gr/display/ESPDInt[ESPDint Business Document].  
...
.. GC files are found in the link:{url-tree}/dist/cl/gc[Validation folder].
