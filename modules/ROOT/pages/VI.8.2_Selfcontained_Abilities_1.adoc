...
In this data structure the first REQUIREMENT set by the contracting authority is whether the criterion is to be WEIGHTED or not (the opposito to WEIGHTED is PASS/FAIL, see the Code List link:{attachmentsdir}/cl/ods/ESPD-CodeLists-V2.0.2[EvaluationMethodType]).
...
. The data structure (see the SELF-CONTAINED spread-sheet tab link:{attachmentsdir}/cl/ods/ESPDRequest-CriteriaTaxonomy-SELFCONTAINED-V2.0.2.ods[SC-Abilities_1]). 
