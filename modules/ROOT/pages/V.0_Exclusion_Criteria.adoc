...
Each branch in this 'criteria taxonomy' corresponds to one data structure that is identical for all the leaves under that branch (e.g. compare the names of the boxes with the tabs of the spreadsheets containing the link:{url-blob}/dist/cl/ods/ESPD-CriteriaTaxonomy-TrackChanges-SELFCONTAINED.ods[Regulated] and link:{url-blob}/dist/cl/ods/ESPD-CriteriaTaxonomy-TrackChanges-SELFCONTAINED.ods[Self-contained] taxonomies).
