...
[arabic]
. In an Office Open XML (link:{attachmentsdir}/cl/xlsx/ESPD-CodeLists-V2.1.1.xlsx[XLSX]) file,
. In an Open Office Document (link:{distUrlTree211}/dist/cl/ods[ODS]) file,
. Genericode 1.0 (link:{distUrlTree211}/dist/cl/gc[GC]) files (one *.gc file per code list).
...
|===
|*REQUIREMENT*: |**For identifiers, this ESPD V3.0.0 specification requires at least (and always) the mandatory attribute **schemeAgencyID. When identifying parties, like the Economic Operator, the Buyer, etc., the attribute schemeID SHOULD also be used (for this use the code list link:{attachmentsdir}/dist/cl/xlsx/[EOIDType]).
|===
...
UBL-2.3 CCT are defined in the file CCTS_CCT_SchemaModule-2.3.xsd (see folders xsd/common or xsdrt/common in the link:{distUrl}/dist/[distribution package]).
...
. Beware that the codes may be numbers, text or combinations of both. These code labels are the ones that are specified in the codelist spreadsheets and XML Genericode files distributed jointly with this specification (in the folder dist/cl of the link:{distUrl}/dist[distribution package].
...
However, this ESP-EDM specification also uses the code list link:{attachmentsdir}/dist/cl/xlsx/[EOIDType] with a different purpose, "the identification of the type of scheme used to identify parties, namely Economic Operators" (but should also used to identify the schemes used to identify Buyers, Service Providers, etc.).
...
. The element cbc:ResponseID is of type Identifier(as defined in the Core Component Type Specification library). The value ''VAT'' assigned to the attribute schemeID, taken from the code list link:{attachmentsdir}/dist/cl/xlsx/[EOIDType], is used to indicate that the type of identifier used is the Value Added Tax identifier issued by the Spanish Tax Agency (ES-AEAT).
