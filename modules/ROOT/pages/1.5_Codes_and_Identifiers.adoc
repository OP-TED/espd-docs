...
.. business-specific domain (i.e. procurement specific, like codes representing contract types, procurement procedure types, criteria types, etc.); 
...
Moreover, the ESPD will reuse code list from (link:https://op.europa.eu/en/web/eu-vocabularies/e-procurement/tables[EU Vocabularies]). This is due to the alignment between ESPD and eForms to ensure the maximum interoperability. With this purpose ESPD will align the use of certain code lists. 
...
However, it is worth to note that those code list that are reused from EU Vocabularies in alignment with eForms are not completely shown in data structure (excel and ods) files. Instead, the metadata and the location of them in EU Vocabularies is provided.

For the genericodes it is different, ESPD will provide the gc files from EU Vocabularies to ensure its use and the validation. In addition, ESPD will also provide GC of the code list purely used and maintained by ESPD.

ESPD-EDM Code lists contain the following elements:
...
image:Values_schemeID_Party_Identifiers.jpg[Elements and metadata of an ESPD code list,width=600,height=264] 
...
_Figure 6. Elements and metadata of a ESPD code list_
...
<cbc:VersionID schemeAgencyID="OP" schemeVersionID="4.4.0">1.0</cbc:VersionID>
----
...
<cbc:ID schemeID="Criterion" schemeAgencyID="OP" schemeVersionID="4.4.0">1fa05728-308f-43b0-b547-c903ffb0a8af</cbc:ID>
...
<cbc:ValidatedCriterionPropertyID schemeID="Criterion" schemeAgencyID="OP" schemeVersionID="4.4.0">1fa05728-308f-43b0-b547-c903ffb0a8af</cbc:ValidatedCriterionPropertyID>
