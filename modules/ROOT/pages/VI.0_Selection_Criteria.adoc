...
Each branch in this 'criteria taxonomy' corresponds to one data structure that is identical for all the leaves under that branch (e.g. compare the names of the boxes with the tabs of the spreadsheets containing the link:{attachmentsdir}/dist/cl/ods/ESPD-CriteriaTaxonomy-BASIC-V2.1.1.ods[Basic] and link:{attachmentsdir}/dist/cl/ods/ESPD-CriteriaTaxonomy-EXTENDED-V2.1.1.ods[Extended] taxonomies).
