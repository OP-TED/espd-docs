...
*Rule*: Compulsory use of the link:{url-tree-301}/codelists/gc/CriterionElementType.gc[CriterionElementType]. Possible types are 'CAPTION, REQUIREMENT and QUESTION'. If the type is CAPTION or REQUIREMENT no answer is expected from the economic operator and therefore the cbc:ValueDataTypeCode must be set to NONE. Otherwise this value must be set to one of the values defined in the link:{url-tree-301}/codelists/gc/ResponseDataType.gc[ResponseDataType]
