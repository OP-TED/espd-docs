...
<2> Response value to the global QUESTION. The data type is `cbc:ResponseIndicator`, which is consistent with the expected data value code INDICATOR (from the Code List link:{url-blob}/dist/cl/ods/ESPD-CodeLists-V02.00.00.xlsx[ResponseDataType]).
