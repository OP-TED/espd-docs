...
|*File*: |link:{attachmentsdir}/dist/xsdrt/maindoc/UBL-QualificationApplicationRequest-2.2.xsd[dist/xsdrt/maindoc/UBL-QualificationApplicationRequest-2.2.xsd]
...
*Rule*: *Rule*: Compulsory use of attributes listID, listAgencyName and listVersionID. Compulsory use of the code list link:{attachmentsdir}/dist/cl/ods/ESPD-CodeLists-V2.1.1.ods[ProcedureType] (values: `Open`, `Restricted`, `Accelerated`, `Competitive dialogue`, etc.). Do not confound with the object of the procurement project (code list `ProjectType`: Works, Supplies, Services).
