...
** *cac:QualifyingParty/cac:Party/cbc:IndustryClassificationCode* to indicate whether the EO is a micro, small, medium or large company (or simply an SME). This code is defined in the Code List link:{url-tree-301}/codelists/gc/EconomicOperatorSize.gc[economic-operator-size]. See also sections "Expected Elements" and "XML example" for more details.
...
The UBL-2.3 element for the role of the economic operator is quite straightforward and typical in UBL: it provides a pair *code + description* (see Code List link:{url-tree-301}/codelists/gc/eo-role-type.gc[eo-role-type] for the codes and descriptions; see also the XML example below).
...
*Rule*: Only the 'Description' is expected, but the component cac:ClassificationScheme offers other rich possibilities (see the link:{url-tree-301}/ubl-2.3[UBL-2.3 model in the distribution package] or in the link:http://docs.oasis-open.org/ubl/csprd01-UBL-2.3/[original source] for more details).
...
*Rule*: Use it to place here the general Turnover of the EO (for statistical purposes). Compulsory assignment of a value to the attribute currency. The default value should be set to 'EUR'. Compulsory use of the Code List link:{url-tree-301}/codelists/gc/Currency.gc[currency].
...
*Information Requirement*: link:http://wiki.ds.unipi.gr/display/ESPDInt/BIS+41+-+ESPD+V2.1.0#BIS41-ESPDV2.1-tbr92-008[_tbr92-008_] *Rule*: Compulsory use of the Code List link:{url-tree-301}/codelists/gc/eo-role-type.gc[eo-role-type].
...
*Rule*: Software applications should retrieve and reuse the description from the Code List link:{url-tree-301}/codelists/gc/eo-role-type.gc[eo-role-type].
...
*Rule*: The country of the contracting body must always be specified. Compulsory use of the Code List link:{url-tree-301}/codelists/gc/Country.gc[Country].
...
*Rule*: The country of the service provider must always be specified. Compulsory use of the Code List link:{url-tree-301}/codelists/gc/Country.gc[Country].
...
[NOTE]
====
. Role of the economic operator. The values are defined in the Code List link:{url-tree-301}/codelists/gc/eo-role-type.gc[eo-role-type]. The selection of the value Sole Tenderer or Group Leader determines whether the data about the relied-on and not-relied-on entities is instantiated in this XML.
...
*Rule*: Compulsory use of the Code List link:{url-tree-301}/codelists/gc/Country.gc[Country].
