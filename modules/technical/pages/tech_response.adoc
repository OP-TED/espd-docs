...
|*Definition*: a|
"_A structured electronic business document for providing qualification information in a simplified way through an ESPD when responding to a Call for Tender._" (source: xref:bis41.adoc[ESPD Response transaction BIS41-ESPDV2.1-tbr92-020 (Trdm092)]).
...
*Information Requirement*: xref:tbr92_reqs.adoc#versioning[tbr92-020].
...
*Rule*: Other documents, e.g. the tender, might refer to the ESPD Response using this identifier (thus it being compulsory). Copies of a document must be identified with a different UUID. Compulsory use of schemeAgencyID attribute.
...
<!-- The type of the procurement procedure; this information is provided by eForms and the concrete notice per procedure. e.g. open = In open procedures any interested economic operator may submit a tender in response to a call for competition.-->
...
[cols=",",]
|===
|*REQUIREMENT* |**The economic operator must include in its ESPD Response document a reference to the ESPD Request. For this, the values of the two identifiers **cbc:ID** and **cbc:UUID** of the ESPD Request must be used as the values for the **cbc:ID** and **cbc:UUID* in the ESPD Response cac:AdditionalDocumentReference (to refer to that ESPD Request).*
|===
...
*Rule*: For the ESPD-EDM it is compulsory use of the value ESPD_REQUEST from the Code List link:{url-tree}/codelists/gc/DocRefContentType.gc[_docref-content-type_]. See also the XML example below on how to specify the ESPD Request.
...
|===
|*REQUIREMENT* a|
*One ESPD Response per economic operator*. The ESPD-EDM, as all the previous versions of the ESPD, requires that every economic operator that participates in one tender provide its own ESPD Response document. This affects sole traders that rely on other entities for the execution of the contract (e.g. subcontractors) and groups of economic operators, e.g. a Joint Venture or a Consortium, and all the sub-contractors on which each economic operator of the group relies on to meet the selection criteria.
...
As the whole ESPD-EDM is based entirely on the UBL-2.3 XSD (W3C) Schemas, these schemas are to be used for ESPD.

Nonetheless some data requested in the ESPD are not modelled in UBL-2.3, namely those aiming at purposes going beyond the identification of the economic operator; e.g. data with statistical purposes; or to ensure the transparency of the procurement procedure; other.
...
** The economic operator, identified with the number used in that (pre)qualification system, is linked unequivocally to that (pre)qualification system.
...
_Figure 9. The rich structure of a UBL-2.3 'Classification Scheme' for the representation of taxonomies_
...
_Figure 10. The structure of UBL-2.3 'Capability'_
...
_Figure 11. The structure of the UBL-2.3 Completed Task (used for references)_
...
_Figure 12. The UBL-2.3 Legal Entity Party_
...
. Does the economic operator rely on other entities to fulfil the selection criteria?
...
* The sole contractor or leader will have to produce a complete ESPD;
...
To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "OTHER-EO-SHELTERED".

//image:Sheltered_workshop_social_business.jpg[Sheltered workshop or social business,width=566,height=175]

//_Figure 12. Sheltered workshop or social business - data structure_
...
To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "OTHER-EO-PQS".

//image:Pre-qualification_related_data_structure.jpg[Pre-qualification-related data structure,width=566,height=255]

//_Figure 13. Pre-qualification-related data structure_
...
To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "OTHER-EO-GROUPS".

//image:Group_EO_data_structure.jpg[Group of EO, data structure,width=642,height=219]

//_Figure 14. Group of EO, data structure_
...
To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "EG-Contributions".

//image:Contributions_certificates_data_structure.jpg[Contributions certificates - data structure,width=642,height=170]

//_Figure 15. Contributions certificates_
...
_Figure 13. EO Roles-entities relied on, mock-up_

*Data structure for the entities upon which the EO relies on*:

To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "OTHER-EO-RELIED_ON-ENTITIES".

//image:Reliedon_entities_data_structure.jpg[Relied on entities - data structure,width=642,height=207]

//_Figure 17. Relied on entities - data structure_
...
_Figure 14. EO Roles-entities not relied on, mock-up_

*Data structure for the entities upon which the EO does not relies on*:

To view the criterion, consult the https://github.com/OP-TED/ESPD-EDM/blob/v4.1.0/criterion/ESPD-criterion.xlsx[Criterion File]. The information will be visible in the tab "OTHER-EO-NOT_RELIED_ON-ENTITIES".

//image:Not_reliedon_entities_data_structure.jpg[Not relied on entities - data structure,width=642,height=207]

//_Figure 19. Not relied on entities - data structure_

==== XML Example

This example contains all the data about an economic operator. Beware that the basic identification data is placed into the cac:EconomicOperatorParty component, whilst the rest of the data (namely for statistical purposes) is structured in the data structures described above.
...
. Street and number of the economic operator. Notice that the cac:AddressLine element is used instead of cbc:StreetName and cbc:BuildingNumber. This is because name and number are not split in two fields.
...
_Figure 15. EO representatives, mock-up_
...
_Figure 16. EO representatives Power of Attorney, XSD Schema_

Notice that the largest part of the data is held in the component cac:EconomicOperatorParty/cac:Party/cac:PowerOfAttorney/cac:AgentParty/cac:Person. The figure below shows (in blue) the expected elements. See the table "Expected Elements" and the XML example below for the details (e.g. how to use the cac:Person/cac:Contact element to contact the representative.
...
_Figure 17. EO representatives - element Person, XSD Schema_
...
_Figure 18. EO representatives - element Contact, inside Person, XSD Schema_
...
|*cac:PowerOfAttorney/cbc:Description* |Text |0..n |The short description for the role of the economic operators representative and other detailed information on the representation. a|
...
|===
|*REQUIREMENT* a|
The ESPD Response must include one criterion response (one answer), and only one, linked to one, and only one, criterion property (one QUESTION) copied from the ESPD Request into the ESPD Response document; and to each criterion property added by the economic operator to the ESPD Response.
...
_Figure 19. cac:TenderingCriterionResponse XSD element_
...
_Figure 20. cac:ResponseValue XSD element_
...
_Figure 21. cac:ApplicablePeriod XSD element_
...
_Figure 22. cac:EvidenceSupplied XSD element_
...
|===
|*Components* |*Type* |*Card* |*Description* |*Requirements*
|*cbc:ID* |Identifier |0..1 |A language-independent token, e.g., a number, that allows to identify a criterion response uniquely as well as allows to reference the criterion response in other documents. A criterion response describes how an economic operators fulfils an specific criterion. a|
*Information Requirement*: xref:tbr92_reqs.adoc#eo[tbr92-005], xref:tbr92_reqs.adoc#eo[tbr92-006], xref:tbr92_reqs.adoc#eo[tbr92-007], xref:tbr92_reqs.adoc#eval_crit[tbr92-018].
...
|*cbc:ValidatedCriterionPropertyID* |Identifier |1 |A cross-reference to the criterion properties which is validated through this response expressed as an identifier. a|
*Information Requirement*: xref:tbr92_reqs.adoc#eo[tbr92-005], xref:tbr92_reqs.adoc#eo[tbr92-006], xref:tbr92_reqs.adoc#eo[tbr92-007], xref:tbr92_reqs.adoc#eval_crit[tbr92-018].
...
*Rule*: Recommendation: use a UUID-version 4 number.
...
. The type of data expected here is a free text by the EO (for the Response, a DESCRIPTION maps to the UBL-2.3 element cbc:Description, which is an extension of the xsd:String).
...
<cbc:Description>This Consortium fulfils all the conditions defined by the buyer in the contract notice, and notably
...
. Notice this UUID is identical to the cac:TenderingCriterionProperty one. This is the way the UBL-2.3 Qualification Application Response document links each QUESTION(s) to one response (and only one), or vice versa.
. The economic operator states here that it meets the criteria. Notice that the data element is the UBL-2.3 element cbc:ResponseIndicator, which is an ''semanticisation'' (a specialisation) of cbc:Indicator and therefore corresponds to the type of data expected by the buyer in the Request (in cbc:ValueDataTypeCode).
...
_Figure 23. cac:Evidence XSD diagram_
...
_Figure 24. cac:DocumentReference inside cac:Evidence XSD diagram_
...
_Figure 25. Conviction 'Participation in criminal organisation', mock-up (EO perspective)_
...
*Information Requirement*: xref:tbr92_reqs.adoc#evidence_link[tbr92-017], xref:tbr70_reqs.adoc#contracting_body[tbr70-006], xref:tbr70_reqs.adoc#call_for_tender[tbr70-007]
...
. The name of the issuer of the first document of evidence.
...
. The name of the issuer of the second document of evidence.
