...
For the ESPD Response, the ESPD-EDM uses the UBL-2.4 document named QualificationApplicationResponse.xsd XSD Schema. This schema can be found under the folder link:{url-tree}/ubl-2.4/xsdrt/maindoc[ubl-2.4/xsdrt/maindoc] (or the equivalent documented xsd folder).
...
|*File*: |link:{url-tree}/ubl-2.4/xsdrt/maindoc/UBL-QualificationApplicationResponse-2.4.xsd[ubl-2.4/xsdrt/maindoc/UBL-QualificationApplicationResponse-2.4.xsd]
|===
...
The XML snippet below shows how the beginning of an ESPD Response XML instance looks like. Please generate an ESPD Request and ESPD Response documents on https://docs.ted.europa.eu/espd-demo/[ESPD Demo Site] to see full example.
...
*Rule*: For the ESDP-EDM it is compulsory use of the value ESPD_REQUEST from the Code List link:{url-tree}/codelists/DocRefContentType.gc[_docref-content-type_]. See also the XML example below on how to specify the ESPD Request.
...
*Rule*: Only the 'Description' is expected, but the component cac:ClassificationScheme offers other rich possibilities (see the link:{url-tree}/ubl-2.4[UBL-2.4 model in the distribution package] or in the link:http://docs.oasis-open.org/ubl/csprd01-UBL-2.4/[original source] for more details).
