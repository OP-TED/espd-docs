...
[arabic]
. A criterion *may* have descendent 'sub-criteria'. This is very useful, for example, to define national exclusion criteria that specialise in the EU criteria defined in the Directive;
. A criterion *may* refer to one or several pieces of legislation (EU, national, other);
. A criterion *must* always contain at least one group of 'properties'. Properties may be informative captions; general requirements issued by the Member State or procedure-specific requirements specified by the buyer; and questions addressed to the economic operator;
. A group of properties *must* always specify at least one 'property';
. A group of properties *may* contain one or several 'sub-groups' of properties.

== XSD Schema

The XSD Schema diagram here shows a global view of the UBL 2.3 cac:TenderingCriterion component:
...
Examples can be viewed directly in the xml files provided on https://github.com/OP-TED/ESPD-EDM/tree/v3.3.0/xml-examples[GitHub].
...
The xml structure of the TenderingCriterionPropertyGroup element can be viewed in either the examples provided on GitHub. Search either document to view the many instances of use:

https://github.com/OP-TED/ESPD-EDM/blob/v3.3.0/xml-examples/ESPD-Request.xml

https://github.com/OP-TED/ESPD-EDM/blob/v3.3.0/xml-examples/ESPD-Response.xml
...
. This is the QUESTION that the economic operator needs to respond to (the "Amount" corresponding to the economic of financial requirement) in this example: "Specify the total invoiced amount, taxes included" (cac:TenderingCriterionProperty UUID 13728a54-21e3-4c84-8b11-48666c3d260f).
...
Responses to any element to a requirement, are placed in a <cac:TenderingCriterionResponse>:
...
More instances of this element can be viewed in the sample xml response file: https://github.com/OP-TED/ESPD-EDM/blob/v3.3.0/xml-examples/ESPD-Response.xml
