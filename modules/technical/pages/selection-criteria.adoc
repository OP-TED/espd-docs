...
. First financial ratio block: the Criterion Property used to refer to the response by the economic operator. In the ESPD Response document, the ID of this Criterion Property will be used by the element cac:ValidatedCriterionPropertyID as the means to link the response to the question. See section xref:tech_response.adoc#answering_questions[Answering QUESTIONs] (under ESPD Response) for more details on this.
...
*_For details on the Schematron-based validation solution see section xref:tech_validation.adoc[Validation].*
====
