...
From this point in the document, the tables defining classes and its properties refer frequently to business rules. As stated in the xref:tech_br_espd_profile.adoc[e-Sens BIS document], "[..] business rules are sets of rules that clarify the content of instances by stating mandatory fields, fixed values (like code lists), dependency between fields in the same object, and dependency between different objects".
...
See formal requirements related to the contracting authorities in the e-Sens site: xref:tech_br_request_reqs.adoc#contracting_body[tbr70-001]*
