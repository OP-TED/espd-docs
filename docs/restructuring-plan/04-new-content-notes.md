...
- **Partial Sources**: business/pages/business_info.adoc (brief mentions of buyer/supplier roles in lines 80–160), ESPD-EDM/README.adoc (actor terminology)
...
- **Notes**: Same situation as Introduction index.adoc — the existing tile layout provides no reusable prose. Effectively new content.
...
- **Partial Sources**: ESPD-EDM/criterion/README.adoc (part structure), espd-service-sandbox/espd-edm/parts/ directory (JSON part definitions: part-I-procedure.json, part-III-exclusion.json, etc.)
...
#### reference-implementation-guide.adoc
- **Priority**: Low
- **Persona**: All
...
- **Partial Sources**: technical/tech_architecture.adoc (lines 1–45: architecture overview, distribution package context), technical/tech_dist_pack.adoc (70 lines: distribution package description), ESPD-EDM/README.adoc
- **Notes**: tech_architecture.adoc contributes partial content (mapped as split → getting-started.adoc), but the "getting started" framing — prerequisites, step-by-step first actions — is new. tech_dist_pack.adoc is mapped with disposition "rewrite" to distribution-tooling.adoc, not to this page; however its overview of the package structure provides useful input.
...
- **Notes**: tech_validation.adoc's general validation content (split → common-concepts.adoc) provides partial input. The rest — identifier schemes, party patterns, document reference patterns — must be synthesised from patterns repeated across the monolithic pages. Requires careful extraction and de-duplication.
...
- **Partial Sources**: ESPD-EDM/criterion/espd-edm.json (the source file itself), ESPD-EDM/criterion/README.adoc (brief format description), espd-service-sandbox/js/codelists.js (demonstrates programmatic consumption patterns)
...
- **Partial Sources**: ESPD-EDM/codelists/ directory (genericode files), technical/tech_codelist.adoc (general code list structure — but mapped to Model Reference code-lists.adoc, not here)
- **Notes**: The unified code-lists.adoc in Model Reference covers ALL code lists. This page focuses specifically on criteria-related code lists with usage context and linkage to the criterion data model. Avoids duplication by cross-referencing code-lists.adoc for detailed value tables.
...
- **Partial Sources**: technical/tech_dist_pack.adoc (70 lines — mapped as rewrite → distribution-tooling.adoc), ESPD-EDM/criterion/README.adoc, espd-tools/EAtoJSON/ (tooling pipeline)
...
- **Partial Sources**: espd-conceptual-model/documentation-integration/index.html (EA-exported HTML documentation), espd-conceptual-model/conceptual-model/ESPD_CM.eapx (EA project file), espd-conceptual-model/documentation-integration/README.adoc
...
- **Priority**: Medium
- **Persona**: Member State developer, Commercial ESPD vendor
- **Scope**: Complete catalogue of all ESPD criterion definitions with their property trees, organised by part (exclusion grounds, selection criteria). Drawn from the espd-edm.json data. Provides a navigable reference for looking up specific criteria by ID, name, or category. Cross-references the criterion-data-model-json.adoc page for format documentation.
- **Partial Sources**: technical/tech_common_aspects_for_criteria.adoc (lines 501–867: criterion classification tables, data structure diagrams — mapped as split → criteria-catalogue.adoc), ESPD-EDM/criterion/espd-edm.json
...
- **Partial Sources**: ROOT/pages/release_notes.adoc (mentions v5 changes at a high level), ESPD-EDM/CHANGELOG.adoc (detailed change log)
- **Notes**: The release notes page (mapped to current-release-v5.adoc) covers what changed but not how to migrate. The CHANGELOG.adoc provides raw change data. A structured migration guide with breaking changes, required actions, and a checklist is new content. Critical for implementers upgrading existing systems.
...
- **Partial Sources**: ESPD-EDM GitHub releases page (external), ESPD-EDM/CHANGELOG.adoc (version history)
...
- **Partial Sources**: technical/tech_external_resources.adoc (14 lines — mapped as move-as-is → related-systems.adoc, NOT this page), ESPD-EDM/README.adoc (mentions standards)
- **Notes**: tech_external_resources.adoc is mapped to related-systems.adoc (links to external EU systems). This standards page is different — it focuses on normative specifications rather than related systems. Entirely new content, though URLs and brief descriptions can be sourced from scattered references throughout existing pages.
...
- **Partial Sources**: technical/tech_dist_pack.adoc (distribution package links), ESPD-EDM/README.adoc (repo links), ROOT/pages/release_notes.adoc (download references)
...
|----------------|-------------|-----------------|
| Business Guide | `using-espd-service.adoc` | End-user guidance on accessing and using the live ESPD Service deployment. Written for Procurement policy officers who need to create/manage ESPDs via the web UI. |
| Business Guide | `reference-implementation-guide.adoc` | Developer-oriented overview of the reference implementation. Links to implementation documentation, architecture decisions, and setup instructions authored in espd-service-sandbox. |
...
1. Create `using-espd-service.adoc` and `reference-implementation-guide.adoc` as placeholder pages with:
...
The `criterion-data-model-json.adoc` page (listed under [Section 4: Criteria](#criterion-data-model-jsonmd) above) is the single most important new-content page for the ESPD-EDM v5 documentation. This section provides additional authoring guidance.
...
- **No existing documentation**: The JSON format has a brief README in `ESPD-EDM/criterion/README.adoc` but no structured documentation explaining the schema, traversal patterns, or consumption guidance.
...
| `ESPD-EDM/criterion/README.adoc` | Brief description of the format and its purpose |
